...
export const SkillsSection: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-20 sm:py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      {/* Section Header */}
      <motion.div
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
        {skillCategories.map((group, groupIdx) => (
          <motion.div
            ...
            className="p-5 sm:p-8 rounded-3xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 hover:border-[#1B4DFF]/40 dark:hover:border-[#FF5A1F]/40 transition-all shadow-sm"
          >
            {/* Category Title */}
            <div className="flex items-center justify-between mb-5 sm:mb-6 pb-3 sm:pb-4 border-b border-black/10 dark:border-white/10">
              <div className="flex items-center gap-3">
                <div className="p-2 sm:p-2.5 rounded-2xl bg-white dark:bg-black/30 border border-black/10 dark:border-white/10 shrink-0">
              ...
              <span className="text-[11px] sm:text-xs font-mono text-[#16161C]/60 dark:text-[#FBFAF7]/60">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-2.5">
              {group.skills.map((skill, skillIdx) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.3, delay: groupIdx * 0.08 + skillIdx * 0.02 }}
                  whileHover={{ scale: 1.03 }}
                  className="group flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl bg-white dark:bg-white/10 text-[#16161C] dark:text-[#FBFAF7] border border-black/10 dark:border-white/10 hover:border-[#1B4DFF] dark:hover:border-[#FF5A1F] shadow-xs font-mono text-[11px] sm:text-xs font-semibold transition-all cursor-default"
                >
                  <CheckCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#1B4DFF] dark:text-[#FF5A1F] opacity-80 group-hover:opacity-100 shrink-0" />
